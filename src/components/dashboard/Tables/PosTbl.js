...
const PosTbl = () => {
    ...
    const record = posData.slice(frstIndex, lasIndex)
    const npage = Math.ceil(posData.length / rcrdprpg)
    ...
    const csvData = [
        ...
        ...posData.map(({ Username, Name, Role, Email }) => [
            Username,
            Name,
            Role,
            Email
        ...
        <div>
            ...
            <div className='flex  flex-col md:flex-row  items-center justify-center mt-3 md:justify-between mx-5'>
                ...
                <div className='flex items-center justify-center my-2 md:my-0'>
                    ...
                    <button onClick={() => { handleExportExcl(posData) }} className='flex border-[1px] px-2 py-1 hover:bg-gray-400 border-gray-600 bg-gray-200 '>
                    ...
                    <tbody >
                        {record.map((value, index) => {
                            ...
                            return <tr key={index} className=''>
                                ...
                                {col9 && <td className="px-1 py-1">{value.totalSaleAmount}</td>}
                                {col10 && <td className=" py-1 px-1">{value.amount}</td>}
                                {col11 && <td className=" py-1 px-1">{(value.totalSaleAmount - value.amount) >= 0 ? value.totalSaleAmount - value.amount : ""}</td>}
