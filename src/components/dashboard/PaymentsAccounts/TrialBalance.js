...
const TrialBalance = () => {
    ...
    const findTotal = () => {
        let total = 0
        trialBalanceData.map(val => {
            return total += val.openingBalance    ////////Customer due only also should be added here
        })
        return total
    }
        ...
        <div className='flex flex-col mt-4 w-full bg-white rounded-md px-3 items-start justify-start'>
            ...
            <div className='flex flex-col w-full mt-10 ' ref={printRef}>
                <table className='table-auto w-full'>
                    ...
                    <tbody >
                        {trialBalanceData.map((sheet) => (

                            <tr >
                                <td>{sheet.name}</td>
                                <td>₨ {sheet.openingBalance}</td>
                                <td></td>
                            </tr>
                        ))}
